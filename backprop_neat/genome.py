...
class DefaultGenome(object):
    ...
    def mutate(self, config):
        ...
        # TODO: weight mutation disabled
        for cg in self.connections.values():
            cg.mutate(config)

        # TODO: bias mutation disabled
    ...
    def add_connection(self, config, input_key, output_key, weight, enabled):
        ...
        if isinstance(weight, torch.Tensor):
            connection.weight = weight.clone().detach().requires_grad_(True)
        else:
            connection.weight = torch.tensor(weight, requires_grad=True)
    ...
    def mutate_delete_node(self, config):
        # Do nothing if there are output nodes.
