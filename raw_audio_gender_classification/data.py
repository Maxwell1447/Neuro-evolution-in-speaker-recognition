    ...
    def __init__(self, subsets, length, stochastic=True, cache=True):
        ...
        for s in subsets:
            ...
            if current_os == "Windows":
                ...
                print("******"+'/speechmaterial/downloads/{}/'.format(s), os.path.isdir('/speechmaterial/downloads/{}/'.format(s)))

                for root, folders, files in os.walk('/speechmaterial/downloads/{}/'.format(s)):
                    subset_len += len([f for f in files if f.endswith('.flac')])
