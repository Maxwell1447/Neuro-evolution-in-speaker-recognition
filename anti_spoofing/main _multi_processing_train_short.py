...
batch_size = 10
batch_num = nb_samples_train // batch_size
...
class Anti_spoofing_Evaluator(neat.parallel.ParallelEvaluator):
    ...
    def evaluate(self, genomes, config):
        ...
        for ignored_genome_id, genome in genomes:
            jobs.append(self.pool.apply_async(self.eval_function, (genome, config, self.next())))
    ...
    def next(self):
        ...
        if self.batch_count > self.batch_num:
            self.batch_count = 0
        for index_audio in range(batch_size * self.batch_count, batch_size * (1 + self.batch_count)):
            self.current_batch.append(self.data.__getitem__(index_audio))
...
def run(config_file, n_gen):
    ...
    multi_evaluator = Anti_spoofing_Evaluator(8, eval_genome, batch_num, train_loader)
