...
from anti_spoofing.utils_ASV import gate_lfcc, show_stats, moving_average, show_stats_no_percentage
...
class Anti_spoofing_Evaluator(neat.parallel.ParallelEvaluator):
    def __init__(self, num_workers, eval_function, data, pop, batch_size=batch_size, timeout=None):
        ...
        self.check_1 = True
        self.check_2 = True
        self.check_1_generation = 0
        self.check_2_generation = 0
    ...
    def evaluate(self, genomes, config):
        ...
        best_fitness_indexes = np.argpartition(F, -10)[-10:]
        F_10_best_average = F[best_fitness_indexes].mean()
        self.list_best_fitness.append(F_10_best_average)
        print(F_10_best_average)
        if len(self.list_best_fitness) >= 10:
            print(moving_average(self.list_best_fitness, 10)[-1])

        if self.check_1 and len(self.list_best_fitness) >= 10 and moving_average(self.list_best_fitness, 10)[-1] >= .9:
            ...
            self.list_best_fitness = []
            self.check_1 = False
            self.check_1_generation = self.nb_generations
        if self.check_2 and len(self.list_best_fitness) >= 10 and moving_average(self.list_best_fitness, 10)[-1] >= .9:
            ...
            self.list_best_fitness = []
            self.check_2 = False
            self.check_2_generation = self.nb_generations
...
def run(config_file, n_gen):
    ...
    multi_evaluator = Anti_spoofing_Evaluator(n_processes, eval_genome, train_short_loader, pop=config_.pop_size)
    winner_ = p.run(multi_evaluator.evaluate, n_gen)

    # Display the winning genome.
    print('\nBest genome:\n{!s}'.format(winner_))

    return winner_, config_, stats_, multi_evaluator.check_1_generation, multi_evaluator.check_2_generation
...
if __name__ == '__main__':
    ...
    check_1_generation_list = []
    check_2_generation_list = []

    for iterations in range(20):
        print("iterations number =", iterations)
        winner, config, stats, check_1_generation, check_2_generation = run(config_path, n_generation)
        ...
        check_1_generation_list.append(check_1_generation)
        check_2_generation_list.append(check_2_generation)

    print("\n")
    print("equal error rate", eer_list_train)
    show_stats(np.array(eer_list_train))

    print("\n")
    print("equal error rate", eer_list_dev)
    ...
    print("\n")
    print("appearance check1", check_1_generation_list)
    show_stats_no_percentage(np.array(check_1_generation_list))

    print("\n")
    print("appearance check2", check_2_generation_list)
    show_stats_no_percentage(np.array(check_2_generation_list))
